...
public class DuplicateMethod extends MethodAdapter
{
	...
	/* Used in visitInsn() */
	public boolean isMultiArray = false;
	public String originalResultTypeDesc = null;
	public String AastoreNewValueDesc = null;
			...
			String desc)
	{
		...
		{ // Syntactic TODOs remove this limitation
		...
		{
			mv.visitFieldInsn(Opcodes.GETSTATIC, fieldsHolderName, Util
		...
		else
		{
			...
			mv.visitFieldInsn(Opcodes.GETFIELD, fieldsHolderName, Util
					.getAddressField(name), ArrayUtil.getTxType(desc)
					.getDescriptor());
		}
	...
	private void addBeforeReadCall(String owner, String name, String desc,
			boolean isStatic)
	{
		if (!isStatic)
		{
			duplicateTargeObject(Opcodes.GETFIELD, owner, null);
			super.visitFieldInsn(Opcodes.GETFIELD, owner, Util
					.getAddressField(name), ArrayUtil.getTxType(desc)
					.getDescriptor());
		}
		else
		{
			super.visitFieldInsn(Opcodes.GETSTATIC, owner, Util
					.getAddressField(name), ArrayUtil.getTxType(desc)
					.getDescriptor());
		}
		...
		super.visitMethodInsn(Opcodes.INVOKESTATIC,
				ContextDelegator.CONTEXT_DELEGATOR_INTERNAL,
				ContextDelegator.BEFORE_READ_METHOD_NAME,
				ContextDelegator.BEFORE_READ_METHOD_DESC_META);
	}
	...
	public void visitInsn(int opcode)
	{
		...
		if (load)
		{
			...
			if (opcode == Opcodes.AALOAD)
			{
				if (isMultiArray)
				{
					super.visitTypeInsn(Opcodes.CHECKCAST, ArrayUtil
							.getArrayContainerType(originalResultTypeDesc)
							.getInternalName());
				}
				else
				{
					super.visitTypeInsn(Opcodes.CHECKCAST,
							Type.getType(originalResultTypeDesc)
									.getInternalName());
				}
			}
			// ..., val* =>
			...
			Label start, Label end, int index)
	{
		...
		if (this.argumentsSize == index + 1 && !addContextToTable)
		{
			...
			super.visitLocalVariable(LOCAL_VARIBALE_NAME, Context.CONTEXT_DESC,
					null, firstLabel, lastLabel, index);
		}

...
public class NonInstrumentedMethodTransformer extends MethodAdapter
{
	...
	public NonInstrumentedMethodTransformer(MethodVisitor mv,
	...
	public void visitCode()
	{
		...
		int i = (insnM.opcode != Opcodes.INVOKESTATIC
				&& insnM.opcode != Opcodes.INVOKESPECIAL ? 1 : 0);
		...
		super.visitMethodInsn(insnM.opcode, insnM.owner, insnM.name,
				insnM.origDesc);
	...
	private void txArrayToArray(Type arrT, MethodVisitor mv)
	{
		...
		{
			// ..., obj* =>
			mv.visitFieldInsn(Opcodes.GETFIELD, ArrayUtil.getTxArrayType(arrT)
					.getInternalName(), "array", Type
		...
		else
		{
			// ..., obj* =>
			mv.visitFieldInsn(Opcodes.GETFIELD, ArrayUtil.getTxArrayType(arrT)
					.getInternalName(), "array", "[" + arrT.getDescriptor());
			// ..., arr =>
		}
		// ..., arr =>
	}
	...
	private void arrayToTxArray(Type arrT, MethodVisitor mv)
	{
		...
		mv.visitMethodInsn(Opcodes.INVOKESPECIAL, arrayType.getInternalName(),
				"<init>", ctorParamDesc);

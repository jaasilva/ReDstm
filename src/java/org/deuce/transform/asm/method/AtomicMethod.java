...
public class AtomicMethod extends MethodAdapter implements Opcodes
{
			...
			String descriptor, Method newMethod, boolean isStatic)
	{
		...
		{
			argumentReolvers[i] = TypeCodeResolverFactory
					.getReolver(argumentTypes[i]);
		}
	...
	{
		final AnnotationVisitor visitAnnotation = super.visitAnnotation(desc,
				visible);
	...
	public void visitCode()
	{
		...
		Label l1 = new Label();
		Label l25 = new Label();
		mv.visitTryCatchBlock(l0, l1, l25,
				AbortTransactionException.ABORT_TRANSACTION_EXCEPTION_INTERNAL); // try{
		Label l2 = new Label();
		mv.visitTryCatchBlock(l0, l1, l2,
				TransactionException.TRANSACTION_EXCEPTION_INTERNAL); // try{
		Label l3 = new Label();
		...
		Label l27 = new Label();
		mv.visitVarInsn(ALOAD, contextIndex);
		mv.visitMethodInsn(INVOKEINTERFACE, Context.CONTEXT_INTERNAL,
				"rollback", "()V");
		mv.visitLabel(l27);
		...
		mv.visitLabel(l17);
		mv.visitVarInsn(ALOAD, contextIndex);
		mv.visitMethodInsn(INVOKEINTERFACE, Context.CONTEXT_INTERNAL, "commit",
				"()Z");
		Label l18 = new Label();
		...
		Label l24 = new Label();
		mv.visitLabel(l24);
		mv.visitLocalVariable("throwable", "Ljava/lang/Throwable;", null, l5,
				l24, throwableIndex);
		mv.visitLocalVariable("context", Context.CONTEXT_DESC, null, l6, l24,
				contextIndex);
		mv.visitLocalVariable("commit", "Z", null, l7, l24, commitIndex);
		if (returnReolver != null)
			mv.visitLocalVariable("result", returnReolver.toString(), null, l8,
					l24, resultIndex);
		mv.visitLocalVariable("i", "I", null, l9, l23, indexIndex);
		mv.visitLocalVariable("ex",
				"Lorg/deuce/transaction/AbortTransactionException;", null, l27,
				l28, exceptionIndex);
		mv.visitLocalVariable("ex",
				"Lorg/deuce/transaction/TransactionException;", null, l13, l14,
				exceptionIndex);
		mv.visitLocalVariable("ex", "Ljava/lang/Throwable;", null, l15, l12,
				exceptionIndex);
	...
	private Label getContext(final int contextIndex)
	{
		...
		mv.visitMethodInsn(INVOKESTATIC,
				ContextDelegator.CONTEXT_DELEGATOR_INTERNAL, "getInstance",
	...
	private Label throwTransactionException()
	{
		...
		mv.visitMethodInsn(INVOKESPECIAL,
				"org/deuce/transaction/TransactionException", "<init>",
				"(Ljava/lang/String;)V");
		mv.visitInsn(ATHROW);
		return label;
	}

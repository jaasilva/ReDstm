...
public class GetStartLists
{
	 ...
	 * =====================
	 */
	public static void getStartLists(int myId, int numThread, Globals glb,
			GetStartLists gsl)
	{
		//
		...
		//
		int numTmpEdge = (5 + (int) (Math.ceil(1.5 * (gsl.GPtr.numIntEdges)
				/ glb.MAX_INT_WEIGHT)));
		...
		{
			(maxIntWtList[i]).startVertex = tmpEdgeList[i
					- i_edgeStartCounter[myId]].startVertex;
			(maxIntWtList[i]).endVertex = tmpEdgeList[i
					- i_edgeStartCounter[myId]].endVertex;
			(maxIntWtList[i]).edgeNum = tmpEdgeList[i
					- i_edgeStartCounter[myId]].edgeNum;
		}
		...
		CreatePartition.createPartition(0, gsl.GPtr.numStrEdges, myId,
				numThread, lss);
		...
		for (i = lss.i_start; i < lss.i_stop; i++)
		{
			 ...
			 * (gsl.GPtr.intWeight[t] == -i) { break; } } int j; for (j = 0; j <
			 * gsl.GPtr.numDirectedEdges; j++) { if (gsl.GPtr.paralEdgeIndex[j]
			 * > t) { break; } } tmpEdgeList[i_edgeCounter].endVertex =
			 * gsl.GPtr.outVertexList[j-1]; tmpEdgeList[i_edgeCounter].edgeNum =
			 * j-1; for (t = 0; t < gsl.GPtr.numVertices; t++) { if
		...
		for (i = i_edgeStartCounter[myId]; i < i_edgeEndCounter[myId]; i++)
		{
			(soughtStrWtList[i]).startVertex = tmpEdgeList[i
					- i_edgeStartCounter[myId]].startVertex;
			(soughtStrWtList[i]).endVertex = tmpEdgeList[i
					- i_edgeStartCounter[myId]].endVertex;
			(soughtStrWtList[i]).edgeNum = tmpEdgeList[i
					- i_edgeStartCounter[myId]].edgeNum;
		}

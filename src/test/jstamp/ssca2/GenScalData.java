...
public class GenScalData
{
	 ...
	 * =========================
	 **/
	public static void genScalData_seq(Globals glb, GraphSDG SDGdataPtr,
			GenScalData gsd, Alg_Radix_Smp radixsort)
	{
		/*
		...
		int estTotCliques = (int) (Math.ceil(1.5 * glb.TOT_VERTICES
				/ ((1 + glb.MAX_CLIQUE_SIZE) / 2)));
		 ...
		 */
		cliqueSizes[totCliques - 1] = glb.TOT_VERTICES
				- lastVsInCliques[totCliques - 2] - 1;
		...
		if (glb.SCALE >= 12)
		{
			estTotEdges = (int) (Math
					.ceil(((glb.MAX_CLIQUE_SIZE - 1) * glb.TOT_VERTICES)));
		}
		else
		{
			estTotEdges = (int) (Math
					.ceil(1.2 * (((glb.MAX_CLIQUE_SIZE - 1) * glb.TOT_VERTICES)
		...
		for (int i_clique = 0; i_clique < totCliques; i_clique++)
		{
			...
			for (int i = 0; i < i_cliqueSize; i++)
			{
				for (int j = 0; j < i; j++)
				{
					float r = (float) (randomPtr.posrandom_generate() % 1000)
							/ (float) 1000;
					if (r >= p)
					{
			...
			if (i_cliqueSize != 1)
			{
				...
				for (int i_paralEdge = 0; i_paralEdge < randNumEdges; i_paralEdge++)
				{
					...
					if ((i != j)
							&& (tmpEdgeCounter[i][j] < glb.MAX_PARAL_EDGES))
					{
						float r = (float) (randomPtr.posrandom_generate() % 1000)
								/ (float) 1000;
						if (r >= p)
		...
		for (int i = 0; i < glb.TOT_VERTICES; i++)
		{
			...
			for (int d = 1; d < glb.TOT_VERTICES; d *= 2, p /= 2)
			{
				...
				if (r <= p)
				{
					...
					if (t1 != t2)
					{
						int randNumEdges = (int) (randomPtr
								.posrandom_generate() % glb.MAX_PARAL_EDGES + 1);
				...
				float r0 = (float) (randomPtr.posrandom_generate() % 1000)
						/ (float) 1000;
		...
		for (int i = i_edgeStartCounter; i < i_edgeEndCounter; i++)
		{
			startVertex[i + numEdgesPlacedInCliques] = startV[i
					- i_edgeStartCounter];
			int a = i + numEdgesPlacedInCliques;
			endVertex[i + numEdgesPlacedInCliques] = endV[i
					- i_edgeStartCounter];
		}
		...
		System.out.println("No. of intra-clique edges - "
				+ numEdgesPlacedInCliques);
		System.out.println("No. of inter-clique edges - "
		...
		for (int i = 0; i < numEdgesPlaced; i++)
		{
			...
			if (r <= p)
			{
				SDGdataPtr.intWeight[i] = (int) (1 + (randomPtr
						.posrandom_generate() % (glb.MAX_INT_WEIGHT - 1)));
			}
			else
			{
		...
		for (int i = 0; i < numEdgesPlaced; i++)
		{
			if (SDGdataPtr.intWeight[i] <= 0)
			{
				...
				{
					SDGdataPtr.strWeight[(-SDGdataPtr.intWeight[i])
							* glb.MAX_STRLEN + j] = (byte) (1 + randomPtr
		...
		radixsort.all_radixsort_node_aux_s3_seq(numEdgesPlaced, startVertex,
				SDGdataPtr.startVertex, endVertex, SDGdataPtr.endVertex);
	...
	public static void genScalData(int myId, int numThread, Globals glb,
			GraphSDG SDGdataPtr, GenScalData gsd, Alg_Radix_Smp radixsort)
	{
		/*
		 * STEP 0: Create the permutations required to randomize the vertices
		 */
		...
		CreatePartition.createPartition(0, glb.TOT_VERTICES, myId, numThread,
				lss);
		...
		/*
		 * TODO : if required #ifdef WRITE_RESULT_FILES Barrier.enterBarrier();
		...
		{
			estTotEdges = (int) (Math
					.ceil(1.0d * ((glb.MAX_CLIQUE_SIZE - 1) * glb.TOT_VERTICES)));
		}
		else
		{
			estTotEdges = (int) (Math
					.ceil(1.2d * (((glb.MAX_CLIQUE_SIZE - 1) * glb.TOT_VERTICES)
							* ((1 + glb.MAX_PARAL_EDGES) / 2) + glb.TOT_VERTICES * 2)));
		}
		...
		if (myId == 0)
		{
			...
			{
				i_edgeEndCounter[i] = i_edgeEndCounter[i - 1]
						+ i_edgeEndCounter[i];
		...
		CreatePartition
				.createPartition(0, numEdgesPlaced, myId, numThread, lss);
		...
		Alg_Radix_Smp.all_radixsort_node_aux_s3(myId, numThread,
				numEdgesPlaced, startVertex, SDGdataPtr.startVertex, endVertex,
				SDGdataPtr.endVertex, radixsort);

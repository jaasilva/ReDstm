...
public class ComputeGraph
{
	 ...
	 * ===================
	 */
	public void prefix_sums(int myId, int numThread, int[] result, int[] input,
			int arraySize)
	{
		int[] p;
		if (myId == 0)
	...
	public void prefix_sumsin(int myId, int numThread, int[] result,
	 ...
	 * ===================
	 */
	public static void computeGraph(int myId, int numThread, Globals glb,
			ComputeGraph computeGraphArgs)
	{
		...
		CreatePartition.createPartition(0, GPtr.numVertices, myId, numThread,
				lss);
		...
		computeGraphArgs.prefix_sums(myId, numThread, GPtr.outVertexIndex,
				GPtr.outDegree, GPtr.numVertices);
		...
		CreatePartition.createPartition(0,
				(GPtr.numVertices * MAX_CLUSTER_SIZE), myId, numThread, lss);
		...
		for (int i = lss.i_start; i < lss.i_stop; i++)
		{
			...
			for (int j = GPtr.outVertexIndex[i]; j < jend; j++)
			{
				...
				if (k == kend)
				{
					...
					atomicMethodThree(GPtr, MAX_CLUSTER_SIZE, impliedEdgeList,
							auxArr, i, v);
		...
		computeGraphArgs.prefix_sumsin(myId, numThread, GPtr.inVertexIndex,
				GPtr.inDegree, GPtr.numVertices);
		...
		for (int i = lss.i_start; i < lss.i_stop; i++)
		{
			for (int j = GPtr.inVertexIndex[i]; j < (GPtr.inVertexIndex[i] + GPtr.inDegree[i]); j++)
			{
				...
				{
					GPtr.inVertexList[j] = impliedEdgeList[i][j
							- GPtr.inVertexIndex[i]];
				}
				else
				{
					GPtr.inVertexList[j] = (auxArr[i])[(j - GPtr.inVertexIndex[i])
							% MAX_CLUSTER_SIZE];
				}
			}
		}
	...
	{
		computeGraphArgs.global_outVertexListSize = computeGraphArgs.global_outVertexListSize
				+ outVertexListSize;
	}
			...
			int maxNumVertices)
	{
		...
		int new_maxNumVertices = ((CreatePartition.MAX(tmp_maxNumVertices,
				maxNumVertices)) + 1);

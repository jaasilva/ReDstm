 ...
 * @since 0.1
 */
public class Benchmark3 implements org.deuce.benchmark.Benchmark
{
	@Bootstrap(id = 3)
	public IntSet m_set;
	int m_range = 1 << 16;
	...
	public org.deuce.benchmark.BenchmarkThread createThread(int i, int nb)
	{
		...
		return new BenchmarkThread3(m_set, m_range, m_rate, max, min);
	}
	...
	public String getStats(org.deuce.benchmark.BenchmarkThread[] threads)
	{
		...
		{
			add += ((BenchmarkThread3) threads[i]).m_nb_add;
			remove += ((BenchmarkThread3) threads[i]).m_nb_remove;
			contains += ((BenchmarkThread3) threads[i]).m_nb_contains;
		}

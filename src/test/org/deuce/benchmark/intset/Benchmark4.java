 ...
 * @since 0.1
 */
public class Benchmark4 implements org.deuce.benchmark.Benchmark
{
	@Bootstrap(id = 3)
	public IntSet m_set;
	int m_range = 1 << 16;
	...
	{
		int id = Integer.getInteger("tribu.site") - 1;
		int reps = Integer.getInteger("tribu.replicas");
		int size = m_range / reps;
		int min = id * size;
		int max = min + size;

		return new BenchmarkThread4(m_set, m_range, m_rate, max, min);
	}
	...
	public String getStats(org.deuce.benchmark.BenchmarkThread[] threads)
	{
		...
		{
			add += ((BenchmarkThread4) threads[i]).m_nb_add;
			remove += ((BenchmarkThread4) threads[i]).m_nb_remove;
			contains += ((BenchmarkThread4) threads[i]).m_nb_contains;
		}

/*
 * RBTreeBenchmark.java Copyright 2008 Sun Microsystems, Inc., 4150 Network
 * Circle, Santa Clara, California 95054, U.S.A. All rights reserved. Sun
 ...
 * more of the U.S. patents listed at http://www.sun.com/patents and one or more
 ...
 * license terms. Sun, Sun Microsystems, the Sun logo and Java are trademarks or
 * registered trademarks of Sun Microsystems, Inc. in the U.S. and other
 * countries. This product is covered and controlled by U.S. Export Control laws
 * and may be subject to the export or import laws in other countries. Nuclear,
 ...
 * reexport to countries subject to U.S. embargo or to entities identified on
 * U.S. export exclusion lists, including, but not limited to, the denied
...
public class RBTree implements IntSet
{
	 ...
	 * transactional.
	 */
	final private RBNode root;
	/**
	 * Used in place of null pointer.
	 */
	public final RBNode sentinelNode;
	...
	// }
	/*
	 * FIXME: This is a big problem as far as I understand. If we are already
	 * inside the scope of a transaction, and a class is loaded whose <clinit>
	 * features a transaction, it will reuse the current Context to start a new
	 * one. This cannot happen, but we can't possibly know if <clinit> is being
		...
		while (x != getRoot() && x.getColor() == /* Color. */BLACK)
		{
			if (x == x.getParent().getLeft()) // determine sub tree from parent
			{
				...
				if (y.getLeft().getColor() == /* Color. */BLACK
						&& y.getRight().getColor() == /* Color. */BLACK)
		...
		while (x != getRoot() && x.getParent().getColor() == /* Color. */RED)
		{
			...
			{ // is it on the Left or Right subtree?
				...
				else
				{
					...
					x.getParent().setColor(/* Color. */BLACK); // make Parent
																// black
	...
	private void recursiveValidate(RBNode root, int blackNodes, int soFar)
	{
		...
		/* Color */int rootcolor = root.getColor();

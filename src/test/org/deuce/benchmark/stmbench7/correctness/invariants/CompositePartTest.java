...
public class CompositePartTest extends InvariantTest
{
	...
	public static void checkInvariants(CompositePart component,
			boolean initial, BaseAssembly aParent,
			TraversedObjects traversedObjects)
	{

		traversedObjects.components.add(component);

		System.err.print("Component " + traversedObjects.components.size()
				+ "\r");

		DesignObjTest.checkInvariants(component, initial,
				Parameters.MaxCompParts, 0, Integer.MAX_VALUE - 1);
		...
		if (aParent != null && !usedIn.contains(aParent))
			reportError(component, id,
					"a BaseAssembly parent not in set usedIn");

		for (BaseAssembly assembly : usedIn)
		{
			if (!assembly.getComponents().contains(component))
				reportError(assembly, assembly.getId(),
						"a child CompositePart not in set of components");
		}

		DocumentationTest.checkInvariants(component.getDocumentation(),
				component, traversedObjects);
		...
		for (AtomicPart part : parts)
			AtomicPartTest.checkInvariants(part, initial, component,
					allConnectedParts, traversedObjects);

		for (AtomicPart part : allConnectedParts)
			if (!parts.contains(part))
				reportError(component, id,
						"a graph-connected AtomicPart not in set of all parts");
	}
}

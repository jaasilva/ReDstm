...
public class IndexTest extends InvariantTest
{
	...
	public static void checkInvariants(Setup setup, boolean initial,
			TraversedObjects traversedObjects)
	{

		// Complex assemblies
		Index<Integer, ComplexAssembly> complexAssemblyIdIndex = setup
		...
		for (ComplexAssembly traversedAssembly : traversedObjects.complexAssemblies)
		{
			...
			checkIndexValue("ComplexAssembly.id", traversedAssembly,
					indexedAssembly, id);
		}
		checkAllTraversed(complexAssemblyIdIndex,
				traversedObjects.complexAssemblies, "ComplexAssembly.id");
		traversedObjects.complexAssemblies.clear();

		// Base assemblies
		Index<Integer, BaseAssembly> baseAssemblyIdIndex = setup
		...
		for (BaseAssembly traversedAssembly : traversedObjects.baseAssemblies)
		{
			...
			checkIndexValue("BaseAssembly.id", traversedAssembly,
					indexedAssembly, id);
		}
		checkAllTraversed(baseAssemblyIdIndex, traversedObjects.baseAssemblies,
				"BaseAssembly.id");
		traversedObjects.baseAssemblies.clear();

		// Composite parts
		Index<Integer, CompositePart> compositePartIdIndex = setup
		...
		for (CompositePart traversedComponent : traversedObjects.components)
		{
			...
			checkIndexValue("CompositePart.id", traversedComponent,
					indexedComponent, id);
		}
		...
		for (CompositePart indexedComponent : compositePartIdIndex)
			if (!traversedObjects.components.contains(indexedComponent))
				CompositePartTest.checkInvariants(indexedComponent, initial,
						null, traversedObjects);
		traversedObjects.components.clear();

		// Documents
		Index<String, Document> documentTitleIndex = setup
		...
		for (Document traversedDocument : traversedObjects.documents)
		{
			...
			checkIndexValue("Document.title", traversedDocument,
					indexedDocument, id);
		}
		checkAllTraversed(documentTitleIndex, traversedObjects.documents,
				"Document.title");
		traversedObjects.documents.clear();

		// Atomic parts (id index)
		Index<Integer, AtomicPart> atomicPartIdIndex = setup
		...
		checkAllTraversed(atomicPartIdIndex, traversedObjects.atomicParts,
				"AtomicPart.id");

		// Atomic parts (buildDate index)
		Index<Integer, LargeSet<AtomicPart>> atomicPartBuildDateIndex = setup
				.getAtomicPartBuildDateIndex();
		for (AtomicPart traversedPart : traversedObjects.atomicParts)
		{
			int id = traversedPart.getId();
			LargeSet<AtomicPart> sameBuildDateParts = atomicPartBuildDateIndex
					.get(traversedPart.getBuildDate());
			if (sameBuildDateParts == null
					|| !sameBuildDateParts.contains(traversedPart))
				reportError("AtomicPart.buildDate", "element with id = " + id
						+ " not in the index");
		}
		...
		for (LargeSet<AtomicPart> sameBuildDateParts : atomicPartBuildDateIndex)
			for (AtomicPart indexedPart : sameBuildDateParts)
				if (!traversedObjects.atomicParts.contains(indexedPart))
					reportError("AtomicPart.buildDate",
							"index contains too many elements");
	...
	private static void checkIndexValue(String index, Object traversedObject,
			Object indexedObject, int elementId)
	{
	...
	private static void checkAllTraversed(Index<?, ?> index,
			HashSet<?> traversedSet, String indexName)
	{
		for (Object indexedObject : index)
			if (!traversedSet.contains(indexedObject))
				reportError(indexName,
						"index contains too many elements (element "
								+ indexedObject.toString() + ")");
	}
}

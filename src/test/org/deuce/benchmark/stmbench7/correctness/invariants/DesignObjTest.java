...
public class DesignObjTest extends InvariantTest
{
	...
	public static void checkInvariants(DesignObj obj, boolean initial,
			int maxId, int minBuildDate, int maxBuildDate)
	{
		int id = obj.getId();
		if (id < 1 || id > maxId)
		...
		if (buildDate < minBuildDate || buildDate > maxBuildDate)
			reportError(obj, id, "buildDate", minBuildDate, maxBuildDate,
					buildDate);
	}

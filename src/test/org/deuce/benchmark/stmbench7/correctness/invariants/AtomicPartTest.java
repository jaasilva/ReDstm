...
public class AtomicPartTest extends InvariantTest
{
	...
	public static void checkInvariants(AtomicPart part, boolean initial,
			CompositePart component, TreeSet<AtomicPart> allParts,
			TraversedObjects traversedObjects)
	{
		...
		if (Math.abs(x - y) != 1)
			reportError(part, id, "inconsistent x and y attributes: x = " + x
					+ ", y = " + y);

		if (part.getPartOf() != component)
			reportError(part, id, "invalid reference to CompositePart parent");

		ImmutableCollection<Connection> to = part.getToConnections(), from = part
				.getFromConnections();
		...
		for (Connection connection : from)
		{
			...
			ImmutableCollection<Connection> sourceToSet = connection
					.getSource().getToConnections();

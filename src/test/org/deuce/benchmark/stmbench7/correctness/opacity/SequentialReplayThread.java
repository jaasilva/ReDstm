...
public class SequentialReplayThread extends BenchThread
{
	...
	public void run()
	{
		...
		for (ReplayLogEntry entry : replayLog)
		{
			System.err.print("Operation " + (opNum++) + " out of " + numOfOps
					+ "\r");
			...
			{
				String opName = OperationId.values()[operationNumber]
						.toString();
				throw new RuntimeError(
						"Different operation result in the sequential execution ("
								+ "operation " + opName + "): "
								+ "Sequential: result = " + result
								+ ", failed = " + failed + ". "
								+ "Concurrent: result = " + entry.result
								+ ", failed = " + entry.failed + ".");
			}
		}
		System.err.println();

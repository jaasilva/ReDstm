...
public class AssemblyBuilder extends DesignObjBuilder
{
	...
	public Assembly createAndRegisterAssembly(Module module,
			ComplexAssembly superAssembly) throws OperationFailedException
	{
	...
	public void unregisterAndRecycleBaseAssembly(BaseAssembly baseAssembly)
	{
		...
		ImmutableCollection<CompositePart> componentsSet = baseAssembly
				.getComponents().clone();
	...
	public void unregisterAndRecycleComplexAssembly(
			ComplexAssembly complexAssembly)
	{
		...
		if (superAssembly == null)
			throw new RuntimeError(
					"ComplexAssemblyFactory: root Complex Assembly cannot be removed!");

		superAssembly.removeSubAssembly(complexAssembly);

		ImmutableCollection<Assembly> subAssembliesSet = complexAssembly
				.getSubAssemblies().clone();
	...
	private BaseAssembly createAndRegisterBaseAssembly(Module module,
			ComplexAssembly superAssembly) throws OperationFailedException
	{
		int date = createBuildDate(Parameters.MinAssmDate,
				Parameters.MaxAssmDate);
		int assemblyId = baseAssemblyIdPool.getId();
		BaseAssembly baseAssembly = designObjFactory.createBaseAssembly(
				assemblyId, createType(), date, module, superAssembly);
	...
	private ComplexAssembly createAndRegisterComplexAssembly(Module module,
			ComplexAssembly superAssembly) throws OperationFailedException
	{
		int id = complexAssemblyIdPool.getId();
		int date = createBuildDate(Parameters.MinAssmDate,
				Parameters.MaxAssmDate);
		ComplexAssembly complexAssembly = designObjFactory
				.createComplexAssembly(id, createType(), date, module,
						superAssembly);
		...
		{
			for (Assembly subAssembly : complexAssembly.getSubAssemblies()
					.clone())

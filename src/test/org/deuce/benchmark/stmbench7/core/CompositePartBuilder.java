...
public class CompositePartBuilder extends DesignObjBuilder
{
	...
	public CompositePartBuilder(
			Index<Integer, CompositePart> compositePartIdIndex,
			...
			Index<Integer, LargeSet<AtomicPart>> partBuildDateIndex)
	{
		...
		atomicPartBuilder = new AtomicPartBuilder(partIdIndex,
				partBuildDateIndex);
	 ...
	 */
	public CompositePart createAndRegisterCompositePart()
			throws OperationFailedException
	{
		...
		if (ThreadRandom.nextInt(100) < Parameters.YoungCompFrac)
			buildDate = createBuildDate(Parameters.MinYoungCompDate,
					Parameters.MaxYoungCompDate);
		else
			buildDate = createBuildDate(Parameters.MinOldCompDate,
					Parameters.MaxOldCompDate);
		...
		CompositePart component = designObjFactory.createCompositePart(id,
				type, buildDate, documentation);
		for (AtomicPart part : parts)
	...
	public void unregisterAndRecycleCompositePart(CompositePart compositePart)
	{
		...
		documentBuilder.unregisterAndRecycleDocument(compositePart
				.getDocumentation());

		for (AtomicPart atomicPart : compositePart.getParts())
			atomicPartBuilder.unregisterAndRecycleAtomicPart(atomicPart);

		ImmutableCollection<BaseAssembly> usedInList = compositePart
				.getUsedIn().clone();
	 ...
	 */
	private void createAtomicParts(AtomicPart[] parts)
			throws OperationFailedException
	{
	...
	private void createConnections(AtomicPart[] parts)
	{
		...
		for (int partNum = 0; partNum < Parameters.NumAtomicPerComp; partNum++)
		{
			...
			{
				int connectTo = ThreadRandom
						.nextInt(Parameters.NumAtomicPerComp);
				parts[partNum].connectTo(parts[connectTo], createType(),
						ThreadRandom.nextInt(Parameters.XYRange) + 1);
			}
		}

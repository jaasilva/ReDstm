...
//		}
		if (Integer.getInteger("tribu.site") == 1) {
			initBank(nb * (Driver.n_threads * Integer.getInteger("tribu.replicas")), init);
		
			System.out.println("Number of accounts  = " + nb * (Driver.n_threads * Integer.getInteger("tribu.replicas")));
	...
	@Atomic
	private void initBank(int nb, float init) {
		if (m_accounts == null) {
			m_accounts = new Account[nb];
			for (int i = 0; i < m_accounts.length; i++) {
				m_accounts[i] = new CheckingAccount("" + i);

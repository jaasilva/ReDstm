...
public class Receiver2 implements Runnable, DeliverySubscriber
{
	...
	public static void main(String[] args)
	{
		...
		{
			threads[i] = new Thread(new Receiver2(i + 1));
			threads[i].start();
		}
	}

	public Receiver2(int x)
	{
		id = x;
	...
	public void run()
	{
		...
		{ // SENDER
			...
			Group g = new PartialReplicationGroup(groupComm.getMembers());
			long st = System.nanoTime();
			for (int i = 0; i < n_msgs; i++)
			{

				groupComm.sendTotalOrdered(
						ObjectSerializer.object2ByteArray(arr), g);

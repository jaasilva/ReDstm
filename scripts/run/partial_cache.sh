...
if [ $1 = "-h" ]; then
	echo "<remote> <threads> <replicas> <run> <writes> <groups> <partial_ops>"
	exit
fi
...
java -Xmx8g -cp $_CP -javaagent:bin/deuceAgent.jar \
	...
	org.deuce.benchmark.Driver -n $_THREADS -d $_DURATION -w $_WARMUP \
		org.deuce.benchmark.partial.bench.Benchmark -r $_RANGE -i $_SIZE \
		-w $_WRITES -po $_PARTIAL_OPS -rr $_REMOTE 2>&1 | tee $_LOG
